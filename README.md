...
## 🚩앞으로 학습할 내용
상품 정보API에서 예매자 리뷰 목록을 반환해 줄 때 리뷰이미지 목록(commentImages)을 service단에서 반복문을 통해 set해주어서 구현하였다. 보통 현업에서는 DB에 조회한 결과를 mybatis(마이바티스), JPA등을 이용하여 객체 멤버 필드와 맵핑한다고하여 향후 mybatis(마이바티스) 또는 JPA를 학습할 예정이다.
